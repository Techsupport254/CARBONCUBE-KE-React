...
/**
 * Generate static HTML files for shop pages with proper meta tags
 * This ensures social media crawlers get shop-specific content
 */
...
function generateShopHTML(shopData) {
    const slug = shopData.enterprise_name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
...
async function generateShopPages() {
    try {
        console.log('🚀 Starting shop page generation...');
        ...
        for (const seller of sellers) {
            try {
                console.log(`📝 Generating page for: ${seller.enterprise_name}`);
                
                // Fetch shop meta data
                const slug = seller.enterprise_name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
        ...
        console.log('🎉 Shop page generation completed!');
