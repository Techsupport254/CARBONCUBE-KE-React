...
function analyzePerformance() {
	try {
		...
		if (!fs.existsSync(reportPath)) {
			console.log(
				'❌ Lighthouse report not found. Run "npm run lighthouse" first.'
			);
			return;
		}
		...
		if (mainThread) {
			...
			console.log(
				`   Status: ${
					mainThread.score >= 0.9
						? "Good"
						: mainThread.score >= 0.5
						? "⚠️ Needs Improvement"
						: "❌ Poor"
				}`
			);
		...
		if (opportunities.length > 0) {
			opportunities.forEach((opportunity) => {
				console.log(`\n• ${opportunity.title}`);
				console.log(
					`  Potential savings: ${opportunity.details.overallSavingsMs}ms`
				);
			});
		} else {
		...
		const criticalIssues = Object.values(audits).filter(
			(audit) => audit.score !== null && audit.score < 0.5
		);
