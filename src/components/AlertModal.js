	...
	useEffect(() => {
		if (isVisible) {
			Swal.fire({
				...
				returnFocus: false,
				didOpen: () => {
					// Remove aria-hidden from root element to prevent accessibility issues
				...
				willClose: () => {
					// Ensure root element doesn't have aria-hidden when closing
					const rootElement = document.getElementById("root");
					if (
						rootElement &&
						rootElement.getAttribute("aria-hidden") === "true"
				...
				document
					.querySelectorAll(
						".swal2-container, .swal2-backdrop, .futuristic-swal"
					)
				...
				// Ensure root element doesn't have aria-hidden after modal closes
				const rootElement = document.getElementById("root");
