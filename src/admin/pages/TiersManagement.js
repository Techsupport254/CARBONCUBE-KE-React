...
const TiersManagement = () => {
		...
		try {
			...
			setTiers(
				data.map((tier, index) => ({
					...
					tier_pricings: tier.tier_pricings || [
	...
	const handleShowModal = (tier = null) => {
		...
		setNewTier(
			tier
				? {
						...tier,
						features: (tier.tier_features || [{ feature_name: "" }]).map(
							(feature) => ({
								id: feature.id,
	...
	const deleteTier = async (tierId) => {
		...
		try {
			const response = await fetch(
				`${process.env.REACT_APP_BACKEND_URL}/admin/tiers/${tierId}`,
				{
					method: "DELETE",
					headers: {
									...
									{newTier.features
										...
										.map((feature, index) => (
											<div
												...
												className="mb-3 px-2 py-1 bg-white rounded shadow-sm d-flex align-items-center"
											>
												<Row className="w-100 g-2">
													...
													>
														<Form.Control
															...
															onChange={(e) => {
																const updatedFeatures = [...newTier.features];
																updatedFeatures[index].feature_name =
																	e.target.value;
																setNewTier({
																	...newTier,
							...
							<Form.Group className="mb-2">
								...
								<div className="pricing-plans-wrapper">
									{newTier.pricings.map((pricing, index) => (
										<div
											key={index}
											className="mb-3 px-2 py-1 bg-white rounded shadow-sm"
										>
											<Row className="g-2 align-items-end">
												<Col md={4} xs={4}>
													<Form.Label className="small mb-0">
														<strong>Duration</strong> (Months)
													</Form.Label>
													<Form.Control
														type="number"
														className="mt-2 mb-1 form-control-md rounded-pill"
														...
														onChange={(e) => {
															...
															updatedPricings[index].duration_months =
																e.target.value;
															setNewTier({
																...newTier,
