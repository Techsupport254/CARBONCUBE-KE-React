...
const CategoriesManagement = () => {
	...
	const handleSaveCategory = async () => {
		...
		try {
			...
			if (!categoryResponse.ok) {
				...
				throw new Error(
					`Failed to save category: ${categoryResponse.status} - ${errorText}`
				);
			}
			...
			const existingSubcategories =
				categories.find((cat) => cat.id === savedCategory.id)?.subcategories ||
				[];
			const savedSubcategoryIds = new Set(
				existingSubcategories.map((sub) => sub.id)
			);
			...
			for (const subcategory of newCategory.subcategories) {
				...
				if (!subcategoryResponse.ok) {
					...
					throw new Error(
						`Failed to save subcategory: ${subcategoryResponse.status} - ${errorText}`
					);
				}
			...
			const newSubcategoryIds = new Set(
				newCategory.subcategories.filter((sub) => sub.id).map((sub) => sub.id)
			);
			const subcategoriesToDelete = Array.from(savedSubcategoryIds).filter(
				(id) =>
					!newSubcategoryIds.has(id) && !newlyCreatedSubcategories.includes(id)
			);
			...
			for (const subcategoryId of subcategoriesToDelete) {
				...
				if (!deleteResponse.ok) {
					...
					throw new Error(
						`Failed to delete subcategory: ${deleteResponse.status} - ${errorText}`
					);
				}
			}
	...
	const handleAddSubcategory = () => {
		setNewCategory({
			...newCategory,
			subcategories: [...newCategory.subcategories, { name: "" }],
		});
	};
