			...
			if (token) {
				try {
					const response = await fetch(
						`${process.env.REACT_APP_BACKEND_URL}/seller/identify`,
						{
							headers: {
		...
		switch (status) {
			...
			case "On-Transit":
				return (
					<FontAwesomeIcon
						icon={faTruckFast}
						className="status-icon on-transit"
					/>
				);
			case "Delivered":
				...
				<Container fluid className="p-0" style={{ flex: 1 }}>
					<Row>
						...
						>
							...
							>
								<Card.Header className="big-card-header justify-content-center ">
									Notifications
								</Card.Header>
								<Card.Body
									className="big-card-body"
											...
											>
												<Card.Body>
													<Card.Title className="d-flex justify-content-between align-items-center">
														<span
															className={`order-id ${getStatusColor(
																notification.status
															)}`}
														>
														...
														<Badge
															bg={
																notification.status === "Processing"
																	...
																	? "success"
																	: notification.status === "On-Transit"
																	? "info"
																	: "success"
															}
														>
