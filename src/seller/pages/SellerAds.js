...
const SellerAds = () => {
	...
	const loadNSFWModel = useCallback(async () => {
		if (!nsfwModelRef.current) {
			...
			// NSFWJS Model Loaded successfully
		}
	}, []);
			...
			reader.onload = async (event) => {
				...
				img.onload = async () => {
					...
					try {
						...
						// NSFW predictions processed successfully
						...
						const neutralOrDrawing =
							(predictionMap["Neutral"] || 0) > 0.6 ||
							(predictionMap["Drawing"] || 0) > 0.6;
						const isUnsafe =
							(predictionMap["Porn"] || 0) > 0.3 ||
							(predictionMap["Hentai"] || 0) > 0.3 ||
							(predictionMap["Sexy"] || 0) > 0.3;
