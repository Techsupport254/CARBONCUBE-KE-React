...
const Orders = () => {
		...
		const fetchOrders = async () => {
			try {
				const response = await fetch(
					`${process.env.REACT_APP_BACKEND_URL}/buyer/orders?buyer_id=${buyerId}&search_query=${searchQuery}`,
					{
						headers: {
	...
	const handleRowClick = async (orderId) => {
		try {
			const response = await fetch(
				`${process.env.REACT_APP_BACKEND_URL}/buyer/orders/${orderId}`,
				{
					headers: {
						Authorization: "Bearer " + sessionStorage.getItem("token"),
					},
