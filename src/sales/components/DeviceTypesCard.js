		...
		<Card className="p-3 shadow-sm custom-card h-100">
			...
			<Card.Body>
				<Row>
					...
					<Col xs={12} md={6}>
						<div className="table-responsive">
							<table className="table table-sm">
								...
								<tbody>
									{Object.entries(deviceAnalytics.device_types || {}).map(
										([device, count]) => {
											const percentage =
					...
					{/* Pie Chart */}
					<Col xs={12} md={6}>
						{Object.keys(deviceAnalytics.device_types || {}).length > 0 ? (
							<Pie
								data={{
									labels: Object.keys(deviceAnalytics.device_types || {}).map(
										(device) => device.charAt(0).toUpperCase() + device.slice(1)
									),
									datasets: [
										{
											data: Object.values(deviceAnalytics.device_types || {}),
											backgroundColor: [
												"#3B82F6",
												"#10B981",
												"#F59E0B",
